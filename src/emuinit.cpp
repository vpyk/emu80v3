...
static unsigned char acDefMons[6]={1,0,1,0,1,2}; // ћониторы по умолчанию
static unsigned char acMaxMonitor[6]={3,0,2,0,4,3}; //  ол-во ћониторов
//static unsigned char acDefMons[7]={1,0,1,0,1,2,1}; // ћониторы по умолчанию
//static unsigned char acMaxMonitor[7]={3,0,2,0,4,3,2}; //  ол-во ћониторов
...
static char *ppcFileErrMess[1]={"Ои®°™† д†©ЂЃҐЃ© Ѓѓ•а†ж®®"};
static char *ppcFormatErrMess[1]={"Н•ѓЃ§§•а¶®Ґ†•ђ†п ђЃ§•Ђм ™Ѓђѓмов•а†"};
static char *ppcMonitorWarnMess[2]={"  Н•®ІҐ•бв≠л© в®ѓ МЃ≠®вЃа†",
                                    "ИбѓЃЂмІг•вбп в®ѓ ѓЃ гђЃЂз†≠®о"};
static char *ppcFileSuccMess[1]={"Ф†©Ђ бЃеа†≠•≠"};
static char *ppcGeneralErrMess[1]={"Ф†в†Ђм≠†п Ѓи®°™†"};
static char *ppcROMErrMess[1]={"Н• ≠†©§•≠ Ѓ§®≠ ®І д†©ЂЃҐ ПЗУ (rom*.bin)"};
static char *ppcSymGenErrMess[1]={"Н• ≠†©§•≠ д†©Ђ І≠†™Ѓ£•≠•а†вЃа† (symgen.bin)"};
static char *ppcFontsErrMess[1]={"Н• ≠†©§•≠ д†©Ђ иа®двЃҐ (fonts/*.bmp)"};
static char *ppcROMDiskErrMess[1]={"Н• ≠†©§•≠ д†©Ђ ROMDISK'†"};

void GeneralError()
{
ShowMessage(ppcGeneralErrMess,1," Ои®°™† ",0,0);
}

void SymGenError()
{
ShowMessage(ppcSymGenErrMess,1," Ои®°™† ",0,0);
}

void FontsError()
{
ShowMessage(ppcFontsErrMess,1," Ои®°™† ",0,0);
}

void ROMDiskError()
{
ShowMessage(ppcROMDiskErrMess,1," Ои®°™† ",0,0);
}

void ROMError()
{
ShowMessage(ppcROMErrMess,1," Ои®°™† ",0,0);
}

void FileError()
{
ShowMessage(ppcFileErrMess,1," Ои®°™† ",0,0);
}

void FormatError()
{
ShowMessage(ppcFormatErrMess,1," Ои®°™† ",0,0);
}

void MonitorWarning()
{
ShowMessage(ppcMonitorWarnMess,2," Па•§гѓа•¶§•≠®• ",0,0);
}
...
static char *ppcROMSList[4]={" О а ® £ ® ≠ † Ђ м ≠ л © ",
                             " У Ђ г з и • ≠ ≠ л © ",
                             " О ™ Ѓ ≠ ≠ л © ",
                             " П К \" Л И К \" ",
                              };

static char *ppcROMPList[2]={" М Ѓ ≠ ® в Ѓ а - В . 0 2 ",
                             " М Ѓ ≠ ® в Ѓ а - В . 0 3 ",
                              };

//static char *ppcROMM80List[2]={" О а ® £ ® ≠ † Ђ м ≠ л © ",
//                               " С Ѓ Ґ ђ • б в ® ђ л ©   б  Р К - 8 6 ",
//                                };
/*
...
int res=ShowMessage(ppcModelList,8," Вл°Ѓа в®ѓ† ПК ",1,cModel);
if (res!=-1)
  {
  ClearDisabledMenuItems();
//  SetDefParams(); //!!!
...
//StopSound(); //!!!!!
if (cModel==MODEL_O)
  res=ShowMessage(ppcROMOList,3," Вл°Ѓа МЃ≠®вЃа† ",1,cMonitor-1)+1;
else if (cModel==MODEL_R)
  {
  res=ShowMessage(ppcROMRList,3," Вл°Ѓа МЃ≠®вЃа† ",1,
  (cMonitor>2)?(cMonitor-1):(cMonitor-1)^1);
  res=res>1?res+1:(res^1)+1;
  }
else if (cModel==MODEL_S)
  {
  res=ShowMessage(ppcROMSList,4," Вл°Ѓа МЃ≠®вЃа† ",1,
  (cMonitor>2)?(cMonitor-1):(cMonitor-1)^1);
  res=res>1?res+1:(res^1)+1;
  }
else if (cModel==MODEL_P)
  res=(ShowMessage(ppcROMPList,2," Вл°Ѓа МЃ≠®вЃа† ",1,(cMonitor-1)^1)^1)+1;
//else if (cModel==MODEL_M80)
//  res=ShowMessage(ppcROMM80List,2," Вл°Ѓа МЃ≠®вЃа† ",1,cMonitor-1)+1;
if (res>0)
  {
  cMonitor=res;
  SetModel(); //##### Ћучше просто изменить им€ файла с ѕ«”
  LoadROM();
  Init(); //!!!
  }
return (res>0);
...
// «агружает параметры из cfg-файла
void LoadCfgParams()
{
char s[81];
FILE *f;
...
static char *ppcHelp[19]={" ГЃапз®• ™Ђ†Ґ®и® Ґ а•¶®ђ• нђгЂпж®®: ",
                         " ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~",
                         "  F10 ........ М•≠о",
                         "  F9  ........ Вл°Ѓа ™Ѓђѓмов•а† ",
                         "  Alt-F9  .... Вл°Ѓа °•І Ѓз ѓ†ђ. ",
                         "  Alt-F8  .... Вл°Ѓа МЃ≠®вЃа† ",
                         "  Alt-F10 .... ВлеЃ§ ",
                         "  Alt-F11 .... С°аЃб ",
                         "  F12 ........ П†гІ†/ҐЃІЃ°≠ЃҐЂ•≠®• ",
                         "  Alt-F1 ..... Эв† ѓЃ§б™†І™†",
                         "  Alt-F2 ..... З†ѓ®бм snapshot'† ",
                         "  Alt-F3 ..... З†£агІ™† (г≠®Ґ•аб.) ",
                         "  Alt-F5 ..... Вл°Ѓа §®б™† A: ",
                         "  Alt-F6 ..... Вл°Ѓа §®б™† B: ",
                         "  Alt-PgUp ... Уб™Ѓа•≠®• а†°Ѓвл ",
                         "  Alt-PgDn ... З†ђ•§Ђ•≠®• а†°Ѓвл ",
                         "  ScrollLock . П•а•™Ђ. QWERTY/ЙЦУКЕН ",
                         "  Alt-Enter .. ПЃЂ≠Ѓн™а./О™Ѓ≠≠л©  ",
                         "  Alt-A ...... О ѓаЃ£а†ђђ•  ",
                         };
...
{
//EnterInteractiveMode();
ShowMessage(ppcHelp,19," ПЃ§б™†І™† ",0,0);
}

void About()
{
ShowMessage(ppcAbout,2," О ѓаЃ£а†ђђ• ",0,0);
}
...
int res=ShowMessage(pszFiles,nFiles," Вл°Ѓа д†©Ђ† ",1,0,1,70,25,fnGetColor);
